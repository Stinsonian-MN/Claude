    ...
    <div style={{fontFamily:"'DM Sans', system-ui, sans-serif", background:"#f5f3ee", minHeight:"100vh", color:"#0f1117"}}>
      ...
      {view === "build" ? (
        <div style={{display:"grid", gridTemplateColumns:"220px 1fr 320px", gap:0, height:"calc(100vh - 65px)"}}>
          <div style={{background:"#ede9e1", borderRight:"1px solid #d4cfc5", overflowY:"auto", display:"flex", flexDirection:"column"}}>
                ...
                return (
                  <div key={key} onClick={() => setActiveTab(key)}
                    style={{padding:"9px 14px", cursor:"pointer", display:"flex", alignItems:"center", gap:9,
                      ...
                      borderBottom:"1px solid #e2ddd4", transition:"background 0.1s"}}
                    ...
                    {count > 0 && <span style={{fontSize:10, fontFamily:"monospace", background:t.bg, color:t.color, padding:"2px 6px", borderRadius:3, fontWeight:700}}>{count}</span>}
                  </div>
                );
              })}
            </div>
            <div style={{padding:"12px 14px", borderTop:"1px solid #d4cfc5", marginTop:"auto"}}>
              ...
              {[{key:"name",label:"Name",type:"text"},{key:"address",label:"Address",type:"text"},{key:"phone",label:"Phone",type:"tel"},{key:"rep",label:"Sales Rep",type:"select"},{key:"date",label:"Date",type:"date"}].map(f => (
          ...
          <div style={{overflowY:"auto", background:"#f5f3ee"}}>
            {TRADE_KEYS.filter(k => k === activeTab).map(key => {
              ...
              return (
                <div key={key}>
                  <div style={{padding:"13px 24px 11px", borderBottom:`2px solid ${t.color}`, background:"#fff", position:"sticky", top:0, zIndex:5, display:"flex", alignItems:"center", gap:10}}>
                    <div style={{width:10, height:10, borderRadius:"50%", background:t.color}} />
                    <span style={{fontFamily:"'DM Serif Display', Georgia, serif", fontSize:18, fontWeight:400}}>{t.label}</span>
                    <span style={{fontFamily:"monospace", fontSize:10, color:"#a8aab2", marginLeft:4}}>{getAllItems(t).length} items</span>
                  </div>
                  {t.sections.map((sec, si) => (
                    <div key={si}>
                      <div style={{padding:"8px 24px 6px", background:t.bg, borderBottom:`1px solid ${t.color}22`, borderTop: si > 0 ? "1px solid #e2ddd4" : "none", display:"flex", alignItems:"baseline", gap:10}}>
                      ...
                      {sec.items.map((item, ii) => {
                        ...
                        return (
                          <div key={item.id} onClick={() => addItem(item)}
                            style={{display:"flex", alignItems:"center", gap:12, padding:"8px 24px 8px 20px",
                            ...
                            onMouseEnter={e => { if(!inList) { e.currentTarget.style.background=t.bg; e.currentTarget.style.borderLeftColor=t.color+"88"; }}}
                            onMouseLeave={e => { if(!inList) { e.currentTarget.style.background=isAlt?"#eeeae4":"#f5f3ee"; e.currentTarget.style.borderLeftColor="transparent"; }}}>
                            <div style={{width:20, height:20, borderRadius:"50%", flexShrink:0, background: inList ? t.color : "#d4cfc5", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, color:"#fff", fontWeight:700}}>
                              {inList ? inList.qty : ""}
                            </div>
                            <div style={{flex:1, minWidth:0}}>
                              <div style={{fontSize:13, color: inList ? t.color : "#1a1c22", fontWeight: inList ? 600 : 400, lineHeight:1.35}}>{item.label}</div>
                              {item.note && <div style={{fontSize:11, color:"#72757f", marginTop:1, lineHeight:1.3}}>{item.note}</div>}
                            </div>
                            <div style={{display:"flex", alignItems:"center", gap:8, flexShrink:0}}>
                              <span style={{fontSize:10, fontFamily:"monospace", color:"#72757f", background:"#e2ddd4", padding:"2px 7px", borderRadius:3}}>{item.unit}</span>
                              <span style={{fontFamily:"monospace", fontSize:13, fontWeight:700, color: inList ? t.color : "#1a4a8a", minWidth:64, textAlign:"right"}}>{fmt(item.price)}</span>
                            </div>
                          </div>
                        );
                      })}
          ...
          <div style={{background:"#fff", borderLeft:"1px solid #d4cfc5", display:"flex", flexDirection:"column", height:"100%"}}>
            <div style={{padding:"14px 18px 12px", borderBottom:"1px solid #e2ddd4", background:"#ede9e1"}}>
              ...
              <div style={{fontFamily:"'DM Serif Display', Georgia, serif", fontSize:28, fontWeight:400, color:"#0f1117", marginTop:2, lineHeight:1}}>{fmt(total)}</div>
              {lineItems.length > 0 && <div style={{fontSize:11, color:"#72757f", marginTop:4, fontFamily:"monospace"}}>{lineItems.length} line item{lineItems.length !== 1 ? "s" : ""}</div>}
            </div>
                  ...
                  <div key={l.id} style={{padding:"10px 16px", borderBottom:"1px solid #f0eeea", borderLeft:`3px solid ${t.color}`}}>
                    ...
                    <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", gap:8}}>
                      <div style={{display:"flex", alignItems:"center", gap:5, flexWrap:"wrap"}}>
                        ...
                        <input type="number" value={l.qty} min="0" step="0.5" onChange={e => updateQty(l.id, e.target.value)} onClick={e => e.stopPropagation()}
                        ...
                        <div style={{position:"relative"}}>
                          ...
                          <input type="text" value={l.customPrice ?? l.price} onChange={e => updatePrice(l.id, e.target.value)} onClick={e => e.stopPropagation()}
                      ...
                      <div style={{textAlign:"right", flexShrink:0}}>
                        ...
                        <button onClick={() => removeItem(l.id)} style={{background:"none", border:"none", cursor:"pointer", fontSize:10, color:"#c0302e", marginTop:2, padding:0, fontFamily:"monospace"}}>✕ remove</button>
                      </div>
                    </div>
                  </div>
                );
            ...
            <div style={{borderTop:"2px solid #d4cfc5", padding:"13px 16px", background:"#f9f8f5"}}>
              ...
              {[{label:"Deductible",val:deductible,set:setDeductible},{label:"Non-Recoverable Dep.",val:nonRecoverable,set:setNonRecoverable}].map(f => (
              ...
              <div style={{borderTop:"1px solid #d4cfc5", marginTop:8, paddingTop:8}}>
                {[{label:"Contract total",val:total,bold:true,color:"#1a4a8a"},{label:"Ins. funds est.",val:Math.max(0,insuranceFunds),bold:false,color:"#1a6640"},{label:"Out-of-pocket",val:outOfPocket,bold:false,color:"#8a4e00"}].map(row => (
              ...
              <button onClick={() => setView("summary")} style={{width:"100%", marginTop:10, padding:"9px", background:"#1a4a8a", color:"#fff", border:"none", borderRadius:6, fontSize:11, fontFamily:"monospace", letterSpacing:"0.07em", textTransform:"uppercase", cursor:"pointer", fontWeight:600}}>
                Review & Print →
              </button>
            </div>
          </div>
        ...
        <div style={{maxWidth:820, margin:"0 auto", padding:"32px 24px 80px"}}>
          <button onClick={() => setView("build")} style={{fontFamily:"monospace", fontSize:10, letterSpacing:"0.06em", textTransform:"uppercase", padding:"6px 14px", border:"1px solid #d4cfc5", borderRadius:4, background:"transparent", cursor:"pointer", marginBottom:24, color:"#72757f"}}>← Back to estimate</button>
          <div style={{background:"#0f1117", color:"#fff", padding:"24px 28px", borderRadius:"10px 10px 0 0", borderBottom:"3px solid #1a4a8a"}}>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-end"}}>
              <div>
                ...
                <div style={{fontFamily:"monospace", fontSize:10, color:"#8fa8d0", marginTop:4}}>952-933-4510 · 7391 Bush Lake Rd, Edina MN · stinsonservices.com</div>
              </div>
          ...
          <div style={{background:"#fff", border:"1px solid #d4cfc5", borderTop:"none"}}>
            {TRADE_KEYS.map(key => {
              ...
              return (
                <div key={key}>
                  <div style={{background:t.bg, padding:"9px 24px", borderBottom:"1px solid #d4cfc5", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                    ...
                    <thead>
                      <tr style={{background:"#f9f8f5"}}>
                        {["Line Item","Unit","Qty","Unit Price","Total"].map((h,i) => (
                          <th key={h} style={{padding:"6px 16px", fontSize:10, fontFamily:"monospace", letterSpacing:"0.07em", textTransform:"uppercase", color:"#a8aab2", textAlign: i > 1 ? "right" : "left", borderBottom:"1px solid #e8e4dc"}}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
          ...
          <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginTop:20}}>
            ...
            <div style={{background:"#fff", border:"1px solid #d4cfc5", borderRadius:8, padding:"18px 20px"}}>
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Scope notes, conditions, special items…"
                style={{width:"100%", minHeight:120, fontSize:12, padding:"10px", border:"1px solid #d4cfc5", borderRadius:6, background:"#f9f8f5", color:"#0f1117", resize:"vertical", fontFamily:"inherit", lineHeight:1.6, boxSizing:"border-box"}} />
            </div>
          </div>
          ...
          <div style={{marginTop:20, textAlign:"center"}}>
            <button onClick={() => window.print()} style={{padding:"12px 32px", background:"#1a4a8a", color:"#fff", border:"none", borderRadius:6, fontSize:12, fontFamily:"monospace", letterSpacing:"0.07em", textTransform:"uppercase", cursor:"pointer", fontWeight:600}}>Print / Save PDF</button>
          </div>
        </div>
      )}
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media print { button { display: none !important; } body { background: white !important; } }
      `}</style>
    </div>
  );
}
